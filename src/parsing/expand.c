...
/*   Updated: 2023/01/06 17:39:56 by eflaquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	expand(t_token *t, t_env *env, int i)
{
	...
	while (tmp_t)
	{
		if (tmp_t->type == WORD)
		{
			...
			{
				if (tmp_t->str[i + 1] && !ft_isalpha(tmp_t->str[i + 1])
					&& tmp_t->str[i + 1] == '$'
					&& (tmp_t->str[i + 1] != '_' || tmp_t->str[i + 1] != '?'))
					tmp_t->str = check_arg_expand(tmp_t->str, &i);
				else if (tmp_t->str[i + 1])
					tmp_t->str = change_expand(tmp_t->str, tmp_e, &i);
			}
		}
